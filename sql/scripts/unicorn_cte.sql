...
WITH folderHierarchyData as 
	...
	folderHierarchyRollup as (
		SELECT 
			...
			f.ResultCount,
			f.MethodCount,
			f.LatestResultCreated,
			f.LatestMethodCreated
		...
		SELECT 
			...
			s.ResultCount,
			s.MethodCount,
			s.LatestResultCreated,
			s.LatestMethodCreated 
	...
	folderRollupCount as (
		select 
			...
			max(s.LatestResultCreated) as LatestReultCreated,
			max(s.LatestMethodCreated) as LatestMethodCreated
		FROM dbo.Folder f
	...
	folderSummary as (
		SELECT 
			...
			fcs.ResultCount,
			fcs.MethodCount,
			fcs.LatestResultCreated,
			fcs.LatestResultCreatedBy,
			fcs.LatestMethodCreated,
			fcs.LatestMethodCreatedBy,
			frc.TotalResultCount,
			frc.TotalMethodCount,
			frc.LatestReultCreated as RollupLatestResultCreated,
			frc.LatestMethodCreated as RollupLatestMethodCreated
		FROM folderHierarchyData fhd
		LEFT JOIN folderCountSummary fcs on fhd.FolderID = fcs.FolderID
		LEFT JOIN folderRollupCount frc on fhd.FolderID = frc.FolderID
	)
select * from folderSummary
